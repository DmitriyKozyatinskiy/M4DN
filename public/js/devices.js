...
function openDeviceForm(event) {
  var id = $(event.target).data('id') || null;
  $.get('/devices/update/' + id, function (response) {
    var title = id ? 'Update device' : 'Add new device';
...
$(document).on('click', '.js-update-device, #js-add-device', openDeviceForm).on('click', '.js-remove-device', removeDevice);
